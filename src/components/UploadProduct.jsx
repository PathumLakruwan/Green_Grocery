...
const UploadProduct = ({ onClose, fetchData }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      toast.error("Quantity, Price, and Selling Price must be valid numbers");
      return;
    }

    if (Number(data.sellingPrice) > Number(data.price)) {
      toast.error("Selling price cannot exceed the original price");
      return;
    }

    const response = await fetch(SummaryApi.uploadProducts.url, {
    ...
    <div className=" fixed w-full h-full top-0 left-0 right-0 bottom-0 flex justify-center items-center ">
      <div className="bg-yellow-500 p-4 rounded-xl w-full max-w-2xl h-full  max-h-[80%] overflow-hidden">
        <div className=" flex justify-between items-center pb-4 ">
          ...
          <div
            className=" w-fit ml-auto font-bold hover:text-red-600 cursor-pointer"
            onClick={onClose}
          >
        ...
        >
                ...
                {data.productImg.map((el, index) => {
                  return (
                    <div className="relative group">
                      ...
                      <div
                        className=" absolute bottom-0 right-0 p-1 bg-red-700 rounded-full text-white text-xs hidden group-hover:block cursor-pointer"
          ...
          <button className="px-4 bg-red-500 text-white mb-10 hover:bg-red-800 rounded-lg">
            Upload Product
          </button>
        </form>
      </div>
